...
class ReportController extends Controller
{
    public function stockReport(Request $request){
        if ($request->ajax()) {
            ...
            if($request->stock_status == 'Stock Out'){
                $query->where('products.purchase_quantity', 'products.selling_quantity');
            }
            ...
            return Datatables::of($products)
                    ...
                    ->editColumn('stock', function($row){
                        if ($row->purchase_quantity - $row->selling_quantity == 0) {
                            return'
                            <span class="badge bg-danger">'.$row->purchase_quantity - $row->selling_quantity.'</span>
                            ';
                        } else {
                            return'
